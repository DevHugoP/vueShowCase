        ...
        <v-col cols="10" offset="1">
          ...
          <div class="date-and-schedule">
            ...
            <div class="d-flex mt-4">
              ...
              <h4
                v-for="(openingHour, index) in exception.openingHours"
                :key="`exception-detail-${index}`"
                :class="index === 0 ? 'ml-3' : 'ml-2'"
              >
                {{ getTimezoneSlotDate(openingHour.start) }} -
                {{ getTimezoneSlotDate(openingHour.end) }}
...
import { ExchangePlaceEntity, FrequencyAvailable } from '@/types'
...
export default mixins(authorizationMixin).extend({
  ...
  computed: {
    ...
    showCapacity() {
      if (this.exception?.slotsFrequency === FrequencyAvailable.CLOSED) {
        return false
      }
    ...
    getEditExceptionDetail() {
      return { name: ROUTES.editExceptions }
    },
  },
  async mounted() {
    try {
      ...
      this.exception = exchangePlace.exceptions![parseInt(this.$route.params.exceptionId, 10)]

      this.exceptions = exchangePlace.exceptions!
  ...
  methods: {
    ...
    getTimezoneSlotDate(date: string) {
      const localDate = dateHelperGetTimezonedDate(
      ...
      return dateHelper.formatToTimezone(localDate, 'UTC', 'p')
    },
  },
})
</script>
